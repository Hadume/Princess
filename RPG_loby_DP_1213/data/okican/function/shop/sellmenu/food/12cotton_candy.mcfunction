...
##合計金額をセット(変えるところ)
    scoreboard players set @s ok_buy 600
...
## アイテム配置
    ...
    #商品(変えるところ)
    item replace entity @n[tag=foodshop] container.13 from block 1 -63 1 container.11
    #戻る
    ...
    #購入する/ミルが不足(変えるところ)
    scoreboard players set @s ok_price 600
    scoreboard players operation @s ok_mill_dummy = @s ok_mill
    execute if score @s ok_mill_dummy matches ..599 run item replace entity @n[tag=foodshop] container.17 with red_stained_glass_pane[custom_model_data=1,custom_name='[{"color":"red","bold": true,"text":"ミルが不足"}]',custom_data={ok_buy:0}] 1
    execute if score @s ok_mill_dummy matches 600.. run item replace entity @n[tag=foodshop] container.17 with green_stained_glass_pane[custom_model_data=1,custom_name='[{"color":"green","bold": true,"text":"購入する"}]',custom_data={ok_buy:0}] 1
