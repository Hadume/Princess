...
## 座標を取得
    ...
    data modify storage temp:asset pos set from storage api: pos

## 引き寄せる
    execute at @s as @e[tag=libInRange] run function asset:magic/52301/cast/mob
...
## 一時使用storageを削除
    data remove storage temp:asset pos
