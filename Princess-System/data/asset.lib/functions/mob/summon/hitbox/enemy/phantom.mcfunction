#> asset.lib:mob/summon/hitbox/enemy/phantom
# ファントムのサイズ
...
## 計算
  ### 幅
    ...
    execute store result storage asset:temp HitBox[0] float 0.5 run scoreboard players get #Size.Copy Temp
  ### 高さ
    execute store result score #Size.Copy Temp run data get storage api: Size
    scoreboard players add #Size.Copy Temp 5
    execute store result storage asset:temp HitBox[1] float 0.5 run scoreboard players get #Size.Copy Temp
