...
## 足りていたら
  execute store success storage asset:magic CanCast byte 1 if score #MP.Copy Temp >= #Magic.MP Asset
## 足りなかったら
  execute unless score #MP.Copy Temp >= #Magic.MP Asset run tellraw @s {"text": "MPが足りないにゃん♡","color": "red"}
