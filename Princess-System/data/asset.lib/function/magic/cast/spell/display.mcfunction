...
## データを用意
	### サイズ
		data modify storage asset:temp transformation.scale set value [0.0f,0.0f,0.0f]
		data modify storage asset:temp transformation.scale[] set from storage asset:magic Range
	### 位置
		data modify storage asset:temp transformation.translation set value [0.0f,0.0f,0.0f]
		...
		#### Y
			...
			execute store result storage asset:temp transformation.translation[1] float -0.00000001 run scoreboard players get #Translation Temp
		#### Z
			execute store result score #Rotation.Y Temp run data get storage api: Rotation[1] 100
			...
			scoreboard players operation #Translation Temp *= #Root2 Temp
			scoreboard players operation #Translation Temp *= #HalfRange Temp
			execute store result storage asset:temp transformation.translation[2] float -0.00000001 run scoreboard players get #Translation Temp
## 回転
	### データを用意
		data modify storage asset:temp transformation.right_rotation set value {axis:[1.0f,0.0f,0.0f],angle:0.0f}
	### 向きを取得
		execute store result score #Rotation.Y Temp run data get storage api: Rotation[1] 100
	### -31400 .. 31400
	...
	###
		execute store result storage asset:temp transformation.right_rotation.angle float 0.0001 run scoreboard players get #Rotation.Y Temp
## 代入
