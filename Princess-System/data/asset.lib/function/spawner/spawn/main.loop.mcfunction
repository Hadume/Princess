...
## リストの何番目を選択するか
    execute store result score #listLength temp if data storage temp: data.Data[]
    execute store result score #rand temp run random value 1..
    scoreboard players operation #rand temp %= #listLength temp
...
##
    execute if score #spawnCount temp matches 1.. run function asset.lib:spawner/spawn/main.loop
