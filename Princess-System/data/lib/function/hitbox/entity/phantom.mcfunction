...
## 計算
	### 幅
		...
		execute store result storage lib: HitBox[0] float 0.1 run scoreboard players get #Size.Copy Temp

	### 高さ
		execute store result score #Size.Copy Temp run data get storage api: Size
		scoreboard players add #Size.Copy Temp 5
		execute store result storage lib: HitBox[1] float 0.1 run scoreboard players get #Size.Copy Temp
