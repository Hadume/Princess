...
## 座標を計算
	...
	### X
		...
		execute store result storage lib:temp Pos1[0] double 0.01 run scoreboard players operation #Pos.X Temp += #Spread Temp
	### Z
		execute store result score #Spread Temp run random value 1..
		scoreboard players operation #Spread Temp %= #Spread.XZ-1 Temp
		scoreboard players operation #Spread Temp -= #Spread.XZ-2 Temp
		execute store result storage lib:temp Pos1[2] double 0.01 run scoreboard players operation #Pos.Z Temp += #Spread Temp
	### Y
		execute store result score #Spread Temp run random value 1..
		scoreboard players operation #Spread Temp %= #Spread.Y-1 Temp
		scoreboard players operation #Spread Temp -= #Spread.Y-2 Temp
		execute store result storage lib:temp Pos1[1] double 0.01 run scoreboard players operation #Pos.Y Temp += #Spread Temp
## 座標を代入
