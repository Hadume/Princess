...
## 基準点の座標を取得
  execute unless data storage lib: Spread.At run function lib:spread/set.at
  execute if data storage lib: Spread.At run data modify storage lib:temp At set from storage lib: Spread.At
## 拡散値を取得
  ### XZ
...
## データを用意
  data modify storage lib: Spread.At1 set value [0.0d,0.0d,0.0d]
## 座標を計算
  execute store result score #Pos.X Temp run data get storage lib:temp At[0] 100
  execute store result score #Pos.Y Temp run data get storage lib:temp At[1] 100
  execute store result score #Pos.Z Temp run data get storage lib:temp At[2] 100
  ### X
    ...
    execute store result storage lib: Spread.At1[0] double 0.01 run scoreboard players operation #Pos.X Temp += #Rand Temp
  ### Y
    execute store result score #Rand Temp run random value 1..
    scoreboard players operation #Rand Temp %= #Spread.2-0 Temp
    scoreboard players operation #Rand Temp -= #Spread.2-1 Temp
    execute store result storage lib: Spread.At1[1] double 0.01 run scoreboard players operation #Pos.Y Temp += #Rand Temp
  ### Z
    execute store result score #Rand Temp run random value 1..
    scoreboard players operation #Rand Temp %= #Spread.1-0 Temp
    scoreboard players operation #Rand Temp -= #Spread.1-1 Temp
    execute store result storage lib: Spread.At1[2] double 0.01 run scoreboard players operation #Pos.Z Temp += #Rand Temp
