...
# 二人とも準備完了だったらトレード実行
    execute if entity @s[tag=UI.TradeReady] if entity @a[tag=UI.TradeSync,tag=UI.TradeReady] run execute as @a if score @s InventoryUI.TradeID = #InventoryUI.Com InventoryUI.TradeID run say 準備完了！
    #function inv.ui:pages/_/com/trade/_/item.trede
