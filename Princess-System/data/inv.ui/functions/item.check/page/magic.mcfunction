#> inv.ui:item.check/page/magic
#   魔法選択から返品？
# @within
#   function inv.ui:item.check/
#   function inv.ui:item.check/page/magic
...
# データ格納
    ...
    execute unless data storage inv.ui: {Player:{Work:{tag:{UiItem:1b}}}} unless score #InventoryUI.Slot InventoryUI.Slot matches 0..8 unless score #InventoryUI.Slot InventoryUI.Slot matches 13 run function inv.ui:item.check/return.item
...
# 再帰処理
    ...
    ## run
        execute if data storage inv.ui: Player.Inventory[-1] run function inv.ui:item.check/page/magic
