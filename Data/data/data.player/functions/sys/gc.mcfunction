...
# 前回存在を確認したIDを移動させる
data modify storage player: id append from storage player: id[0]
data remove storage player: id[0]
...
execute store result score _ PlayerID run data get storage player: id[0]
execute if score _ PlayerID > _ Player run function data.player:sys/gc_loop
scoreboard players operation _ Player >< _ PlayerID

# loop while x - next >= 2(prev - x) - 1 (⇔ 3x - 2prev - next + 1 >= 0)
# 2prev
execute store result score _ Player run data get storage player: id[-1]
